...
const resolvers = {
  Query: {
    ...
    allBooks: async (root, args) => {
      const writer = args.author ? await Author.findOne({ name: args.author }) : null;
      if (args.author && args.genre) {
        return Book.find({ $and: [{ author: writer.id }, { genres: { $in: [args.genre] } }]}).populate("author");
      } else if (args.author) {
        return Book.find({ author: writer.id }).populate("author");
      } else if (args.genre) {
    ...
    me: (root, args, context) => {
      return context.currentAccount || context.currentUser
    }
  },
  Author: {
    name: (root) => root.name,
  ...
  Mutation: {
    addBook: async (root, args, context) => {
      const currentUser = context.currentAccount || context.currentUser
      ...
      try {
        ...
        await book.save();
        await author.save();
        notifier.publish("BOOK_ADDED", { bookAdded: book });
        return book;
      } catch (error) {
  ...
  Subscription: {
    bookAdded: {
      subscribe: () => notifier.asyncIterator('BOOK_ADDED')
    },
  },
};
