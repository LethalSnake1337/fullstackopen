...
const Books = ({ show, selectedGenre, handleGenreChange }) => {
  ...
  if (!show) return null
  if (allBookResult.loading || genreBookResult.loading) return <div>loading...</div>
  if (!allBookResult.data) return <div>Failed to load books</div>
  ...
  if (books.length === 0) return <div>No books to show</div>
